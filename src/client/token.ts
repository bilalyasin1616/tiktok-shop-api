...
class AccessToken {
  ...
  async refreshToken(props: { refresh_token?: string; auto_filled?: boolean }) {
    ...
    // 请求逻辑
    const app_key = this.config?.app_key || ClientConfiguration.globalConfig.app_key;
    const app_secret = this.config?.app_secret || ClientConfiguration.globalConfig.app_secret;
    ...
    return new Promise<{
      ...
      localVarRequest(options, (error, response, body: BaseResponse<TokenResponse>) => {
        if (error) {
          reject(error);
        }

        if (auto_filled) {
          if (body.data?.access_token) this.access_token = body.data?.access_token;
          if (body.data?.refresh_token) this.refresh_token = body.data?.refresh_token;
        }
...
class AccessTokenBuilder {
  static build(access_token: string, refresh_token?: string, config?: ClientConfiguration) {
...
class AccessTokenTool {
  static async refreshToken(refresh_token: string, app_key?: string, app_secret?: string) {
  ...
  static async getAccessToken(auth_code: string, app_key?: string, app_secret?: string) {
    let _app_key = app_key || ClientConfiguration.globalConfig.app_key;
    let _app_secret = app_secret || ClientConfiguration.globalConfig.app_secret;
